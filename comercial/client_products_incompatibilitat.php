<?php include 'includes/header_loged.php'; ?>


  <!-- BEGIN SPECIFIC PAGE CLIENT PRODUCTS -->
  <div class="basic-page cont-wrapper" xmlns="http://www.w3.org/1999/html">
    <div class="page-client-products cont-wrapper">
      <div class="content">
          <?php include 'includes/breadcrumb.php'; ?>

        <h1 class="main-title">Els meus productes</h1>

        <div class="invoices-select">
          <h3>Selecciona el teu compte</h3>
          <select>
            <option>Tots els comptes</option>
            <option>123456790</option>
            <option>123456791</option>
            <option>123456792</option>
            <option>123456793</option>
            <option>123456794</option>
            <option>123456795</option>
          </select>
        </div>
        <a href="#" class="button-pink">Cercar</a>
      </div>
    </div>
    <div class="result-client-products cont-wrapper">
      <div class="content">
        <div class="services-selector opened">
          <div class="services-element">
            <div class="services-title">
              <span class="icons icon-mobile"></span>
              <p>Línia de mòbil</p>
            </div>
            <div class="services-price">
              <p>30 €</p>
            </div>
          </div>
          <div class="services-full-info">
            <div class="config-client-products cont-wrapper">
              <div class="content">
                <div class="sidebar">
                  <div class="prod-title">
                    Serveis inclosos
                  </div>
                  <div class="services-big">
                    <div class="prod-description">
                      Lorem ipsum
                    </div>
                  </div>
                  <div class="services-small">
                    <span class="big">30€</span>
                  </div>
                  <div class="services-container">
                    <div class="producte">
                      <span class="icons icon-call pink">&nbsp;</span>
                      <span class="contactat">Minuts contractats</span>
                    </div>
                    <ul class="info-list-producte">
                      <li>
                        <span>Nacionals</span>
                        <span class="info-detail-producte">240 min.</span>
                      </li>
                      <li>
                        <span>Internacionals</span>
                        <span class="info-detail-producte">100 min.</span>
                      </li>
                      <li>
                        <span>En itinerància</span>
                        <span class="info-detail-producte">100 min.</span>
                      </li>
                    </ul>
                  </div>
                  <div class="services-container">
                    <div class="producte">
                      <span class="icons icon-web pink">&nbsp;</span>
                      <span class="contactat">GB contractats</span>
                    </div>
                    <ul class="info-list-producte">
                      <li>
                        <span>Nacionals</span>
                        <span class="info-detail-producte">2 GB</span>
                      </li>
                      <li>
                        <span>En itinerància</span>
                        <span class="info-detail-producte">500 MB.</span>
                      </li>
                    </ul>
                  </div>
                  <div class="services-container">
                    <div class="producte">
                      <span class="icons icon-email pink">&nbsp;</span>
                      <span class="contactat">SMS disponibles</span>
                      <span class="info-detail-producte">10 SMS</span>
                    </div>
                  </div>
                  <div class="services-container">
                    <div class="services-big"><span
                          class="pin-element">Codi PUK</span>
                    </div>
                    <div class="services-small"><span
                          class="right pink">42882022</span>
                    </div>
                  </div>
                </div>
                <div class="content-ws">
                  <h3>Configuració</h3>

                  <div class="services-container">
                    <ul class="switch-checks-list">
                      <li>
                        <div class="info-check">
                          <span>Activar targeta SIM </span>
                          <span
                              class="little-info-check">En cas de pèrdua de la targeta SIM, desactiva aquesta opció</span>
                        </div>
                        <label class="checkbox-custom-switch">
                          <input type="checkbox">
                          <div class="switch"></div>
                        </label>
                      </li>
                      <li>
                        <div class="info-check">
                          <span>Desactivar bústia de veu</span>
                        </div>
                        <label class="checkbox-custom-switch">
                          <input type="checkbox"
                                 checked>
                          <div class="switch"></div>
                        </label>
                      </li>
                      <li>
                        <div class="info-check">
                          <span>Activar identificació del número</span>
                          <span
                              class="little-info-check">Permet la visualització del teu número quan facis trucades</span>
                        </div>
                        <label class="checkbox-custom-switch">
                          <input type="checkbox">
                          <div class="switch"></div>
                        </label>
                      </li>
                      <li>
                        <div class="info-check">
                          <span>Desactivar itinerància</span>
                          <span class="little-info-check">Si desactives aquesta opció, el mòbil no agafarà cobertura en itinerància, no podràs fer ni rebre trucades, ni SMS, ni dades.</span>
                        </div>
                        <label class="checkbox-custom-switch">
                          <input type="checkbox"
                                 checked>
                          <div class="switch"></div>
                        </label>
                      </li>
                      <li>
                        <div class="info-check">
                          <span>Activar internet nacional</span>
                          <span class="little-info-check">Permet connectar-te a internet des del mòbil només sota cobertura nacional</span>
                        </div>
                        <label class="checkbox-custom-switch">
                          <input type="checkbox">
                          <div class="switch"></div>
                        </label>
                      </li>
                      <li>
                        <div class="info-check">
                          <span>Desactivar internet nacional i en itinerància</span>
                          <span class="little-info-check">Permet connectar-te a internet des del mòbil només sota cobertura nacional i en itinerància</span>
                        </div>
                        <label class="checkbox-custom-switch">
                          <input type="checkbox"
                                 checked>
                          <div class="switch"></div>
                        </label>
                      </li>
                    </ul>
                  </div>

                  <div class="config-box">
                    <h4 class="title-config-box">Necessites
                      un forfet superior?</h4>
                    <div class="inner-box">
                      <div class="custom-item one">
                        <div class="message">
                          Amplia a
                        </div>
                        <div class="big-pink bold">
                          1 GB / Tarifa plana
                        </div>
                      </div>
                      <div class="custom-item two">
                        <div class="full-centered">
                          <br><br>
                          <span class="treinta grey-medium bold">29€</span> <span class="grey-medium bold">/mes</span>
                        </div>
                      </div>
                      <div class="custom-item three">
                        <div class="actions-container">
                          <a href="#"
                             class="button-pink action-sign-in">Contratar</a>
                          <a href="#" class="button-active">+ Info</a>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div class="config-box">
                    <h4 class="title-config-box">Contracta
                      més GB</h4>
                    <div class="inner-box trans">
                      <div class="gallery-item">
                        <div class="product-promo">
                          <div class="big-pink bold">
                            500 MB
                          </div>
                          <div class="entry-price">
                            <span class="twenty grey-medium bold">4€</span> /mes
                          </div>
                        </div>
                        <div class="custom-links">
                          <a href="#" class="button-pink">Contractar</a>
                          <a href="#" class="button-active">+ Info</a>
                        </div>
                      </div>
                      <div class="gallery-item">
                        <div class="product-promo">
                          <div class="big-pink bold">
                            1 GB
                          </div>
                          <div class="entry-price">
                            <span class="twenty grey-medium bold">6€</span> /mes
                          </div>
                        </div>
                        <div class="custom-links">
                          <a href="#" class="button-pink">Contractar</a>
                          <a href="#" class="button-active">+ Info</a>
                        </div>
                      </div>
                      <div class="gallery-item">
                        <div class="product-promo">
                          <div class="big-pink grey-medium bold">
                            2 GB
                          </div>
                          <div class="entry-price">
                            <span class="twenty grey-medium bold">8€</span> /mes
                          </div>
                        </div>
                        <div class="custom-links">
                          <a href="#" class="button-pink">Contractar</a>
                          <a href="#" class="button-active">+ Info</a>
                        </div>
                      </div>

                    </div>
                  </div>

                </div>
              </div>
            </div>
          </div>
        </div>

        <div class="services-selector">
          <div class="services-element">
            <div class="services-title">
              <span class="icons icon-phone"></span>
              <p>Internet i fixe</p>
            </div>
            <div class="services-price">
              <p>49 €</p>
            </div>
          </div>
          <div class="services-full-info">
            <p>Aqui va el contenido de INTERNET I FIXE</p>
            <p>Aqui va el contenido de INTERNET I FIXE</p>
            <p>Aqui va el contenido de INTERNET I FIXE</p>
            <p>Aqui va el contenido de INTERNET I FIXE</p>
          </div>
        </div>

        <div class="services-selector opened">
          <div class="services-element">
            <div class="services-title">
              <span class="icons icon-tv"></span>
              <p>Televisió</p>
            </div>
            <div class="services-price">
              <p>15 €</p>
            </div>
          </div>
          <div class="services-full-info">
            <div class="config-client-products cont-wrapper">
              <div class="content">
                <div class="sidebar">
                  <img src="./images/products/movistar-plus.png">

                  <div class="prod-title">
                    Serveis inclosos
                  </div>
                  <div class="services-big">

                    <div class="prod-description">
                      Mini
                    </div>
                  </div>
                  <div class="services-small">
                    <span class="big">15€</span>
                  </div>

                </div>

                <div class="content-ws">
                  <h3>Configuració</h3>

                  <div class="config-box">
                    <h4 class="title-config-box">Activa
                      Movistar+ als teus dispositius</h4>
                    <div class="inner-box">
                      <h5>Mira la TV on vulguis i quan
                        vulguis</h5>

                      <div class="custom-item one"><img
                            src="./images/products/movistar-plus.png">
                      </div>
                      <div class="custom-item two"><img
                            src="./images/3-devices.png">
                      </div>
                      <div class="custom-item three">
                        <div class="actions-container">
                          <a href="#"
                             class="button-pink action-activate">Activar</a>
                          <a href="#"
                             class="button-active">+
                            Info</a>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div class="config-box">
                    <h4 class="title-config-box">Amplia la
                      teva tarifa</h4>
                    <div class="inner-box">
                      <div class="custom-item one">
                        <img src="./images/products/movistar-plus.png">
                        <div class="big-pink bold">
                          Movistar+ Familiar y cine
                        </div>

                      </div>
                      <div class="custom-item two">
                        <div class="full-centered">
                          <br><br>
                          <span class="treinta grey-medium bold">29€</span> <span class="grey-medium bold">/mes</span>

                        </div>
                      </div>
                      <div class="custom-item three">
                        <div class="actions-container">

                          <a href="#"
                             class="button-pink action-sign-in">Contratar</a>
                          <a href="#" class="button-active">+ Info</a>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>

          </div>
        </div>
      </div>
    </div>

  </div>
  <!-- END SPECIFIC PAGE CLIENT PRODUCTS -->

  <!--ALERT FOR FLOATING DATA-->
  <div class="general-alert-wrapper fixedone">
    <div class="content-general-wrapper clientproducts"> <!---->
      <div class="alert-general-close">
        <i class="icons icon-close"></i>
      </div>
      <div class="alert-general-title"><i class="icons icon-close-rounded"></i>Ho sentim!</div>
      <div class="alert-general-body">
        <div class="incompatible-product">Estimada Alejandra, estàs intentant contractar la mateixa tarifa que ja tens
          contractada.
          Accedeix a la part de “Els meus productes” per veure les teves tarifes contractades.
          <div class="full-centered"><a href="#" class="button-pink">Els meus productes</a></div>
        </div>
      </div>
    </div>
  </div>
  <!--IMPORTANT:: REQUIRES LOAD src="/js/dinamicglobalalert.js" SCRIPT IN HEAD-->


  <script language="JavaScript">
      $(document).ready(function () {

          revealalert();

          // Fold-Unfold ITEMS
          // We start closing ALL (on load PAGE) not marked has "opened"
          $('.services-selector:not(".opened")').children('.services-full-info').hide();

          // Then we configure actions for fold-unfold
          $('.services-element').click(function () {
              var yoSoyTuPadre = $(this).parent('.services-selector');
              yoSoyTuPadre.toggleClass('opened');
              yoSoyTuPadre.children('.services-full-info').slideToggle();
          });

          // ALERTS-ColorBoxes
          var floatingWrapper = $('.general-alert-wrapper'),
              floatingAlert = $('.content-general-wrapper.clientproducts'),
              alertTitle = $('.alert-general-title'),
              alertBody = $('.alert-general-body'),
              scrollTop = $(window).scrollTop();

          // Open ALERT without CONTENT, fill with AJAX callback
          $('.action-activate').click(function (e) {
              e.preventDefault();
              // Call FORM Alta MOVISTAR
              $.post("includes/fake_api.php", {operation: 'altamovistar'})
                  .done(function (data) {
                      alertTitle.html(data.title);
                      alertBody.html(data.body);
                  })
                  .fail(function (data) {
                      alertTitle.html(data);
                      alertBody.html(data);
                  });
              if ($(window).width() < 480) {
                  floatingWrapper.removeClass('fixedone');
                  floatingAlert.css({top: scrollTop});
              }

              // AJAX triggers for ALTA
              $('.clientproducts').on('click', '.activate-mplus', function () {
                  // altamovistar_validation
                  $.post("includes/fake_api.php", {operation: 'altamovistar_validation'})
                      .done(function (data) {
                          alertTitle.html(data.title);
                          alertBody.html(data.body);
                      })
                      .fail(function (data) {
                          alertTitle.html('Error...');
                          alertBody.html('We can\'t process your request now...');
                      });
              });
              $('.clientproducts').on('click', '.mplus-activated', function () {
                  // altamovistar_validation
                  $.post("includes/fake_api.php", {operation: 'altamovistar_finished'})
                      .done(function (data) {
                          alertTitle.html(data.title);
                          alertBody.html(data.body);
                      })
                      .fail(function (data) {
                          alertTitle.html('Error...');
                          alertBody.html('We can\'t process your request now...');
                      });

                  setTimeout(function () {
                      closeAlert();
                  }, 2000);
                  window.location.replace("http://www.at.dev/client_products_tvactiva.php");

              });
          });


          // NOTE: THIS IS THE SAME/Identical as before, but for "CONTRATAR"
          $('.action-sign-in').click(function (e) {
              e.preventDefault();
              // Call FORM Alta MOVISTAR
              $.post("includes/fake_api.php", {operation: 'altamovistar'})
                  .done(function (data) {
                      alertTitle.html(data.title);
                      alertBody.html(data.body);
                  })
                  .fail(function (data) {
                      alertTitle.html(data);
                      alertBody.html(data);
                  });
              if ($(window).width() < 480) {
                  floatingWrapper.removeClass('fixedone');
                  floatingAlert.css({top: scrollTop});
              }
              revealalert();

              // AJAX triggers for ALTA
              $('.clientproducts').on('click', '.activate-mplus', function () {
                  // altamovistar_validation
                  $.post("includes/fake_api.php", {operation: 'altamovistar_validation'})
                      .done(function (data) {
                          alertTitle.html(data.title);
                          alertBody.html(data.body);
                      })
                      .fail(function (data) {
                          alertTitle.html('Error...');
                          alertBody.html('We can\'t process your request now...');
                      });
              });
              $('.clientproducts').on('click', '.mplus-activated', function () {
                  // altamovistar_validation
                  $.post("includes/fake_api.php", {operation: 'altamovistar_finished'})
                      .done(function (data) {
                          alertTitle.html(data.title);
                          alertBody.html(data.body);
                      })
                      .fail(function (data) {
                          alertTitle.html('Error...');
                          alertBody.html('We can\'t process your request now...');
                      });
                  $('.service-status').removeClass('inactive').addClass('active');
                  $('.service-status a').html('Activa');
                  setTimeout(function () {
                      closeAlert();
                  }, 2000);
                  window.location.replace("http://www.at.dev/client_products_tvactiva.php");

              });
          });

          // Close Alert
          $('.clientproducts').on('click', '.close-button', function () {
              closeAlert();
          });


      });
  </script>

<?php include 'includes/footer.php'; ?>